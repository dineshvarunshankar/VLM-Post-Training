...
model, tokenizer = FastModel.from_pretrained(
    ...
    load_in_4bit = False,  # 4 bit quantization to reduce memory (NF4) - This IS QLoRA, not QLoRA
...
trainer = SFTTrainer(
    ...
    args=SFTConfig(
        ...
        report_to="wandb",
        run_name=f"cosmos_reason2_no_cot/lora_{datetime_string}",
        output_dir=f"outputs/cosmos_reason2_no_cot/lora_{datetime_string}",
...
#save the model
model.save_pretrained(f"outputs/cosmos_reason2_no_cot/lora_{datetime_string}/adapter_weights")
tokenizer.save_pretrained(f"outputs/cosmos_reason2_no_cot/lora_{datetime_string}/tokenizer_weights")
...
model.save_pretrained_merged(f"outputs/cosmos_reason2_no_cot/lora_{datetime_string}/fused_model_weights", tokenizer)
