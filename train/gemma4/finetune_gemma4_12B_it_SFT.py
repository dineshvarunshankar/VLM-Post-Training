...
#load the model (instruct 12b-it)
...
trainer = SFTTrainer(
    ...
    args=SFTConfig(
        ...
        report_to="wandb",
        run_name=f"gemma_4_12b_it/lora_{datetime_string}",
        output_dir=f"outputs/gemma_4_12b_it/lora_{datetime_string}",
        save_strategy="steps",
        save_steps=50,
        save_total_limit=3,
...
#save the model
model.save_pretrained(f"outputs/gemma_4_12b_it/lora_{datetime_string}/adapter_weights")
tokenizer.save_pretrained(f"outputs/gemma_4_12b_it/lora_{datetime_string}/tokenizer_weights")
...
model.save_pretrained_merged(f"outputs/gemma_4_12b_it/lora_{datetime_string}/fused_model_weights", tokenizer)

#saving to GGUF/llama.cpp conversion
#model.save_pretrained_gguf(f"outputs/gemma_4_12b_it/lora_{datetime_string}/fused_model_weights", tokenizer, quantization_method = "Q8_0") # Q8_0, BF16, F16 supported